	...
	ObjectMapper mapper = new ObjectMapper();
	 ...
	 if(USERINFO==null)	{
	    	...
	    	$scope.table = $('#table').dataTable( {
	    		fnRowCallback: function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
	    		    ...
	    		    $(nRow).on( 'click', 'td', function () {
	    		    	...
	    		    	if(columnIndex==0){
	    		    		detailMdMember(aData.memberSq);
	    		    	}else if(columnIndex==1){
	    		    		detailMdMember(aData.memberSq);
	    		    	}else if(columnIndex==4){
	    		    		detailMdUnit(aData.memberSq);
	    		    	}
	    ...
	    appControllers.controller('NewController',function($rootScope,$scope,$routeParams,$location,Member,$modal){
	    	//$scope.item={status:'A'};
	    	$scope.item=new Member({'kind':'B','status':'A','profit':40});
		...
		<div class="panel-body">
			...
			<div style="float:right">
				<button type="button" class="btn btn-danger btn-sm" ng-click="onNew()">
					<span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> 매체계정생성
				</button>
	...
	<div class="panel-body">
		<form class="form-horizontal" name="myForm" novalidate>
			...
			<div class="form-group">
    			<label class="col-sm-2 control-label">수익배분</label>
    			<div class="col-sm-10">
      				<input type="number" class="form-control" ng-model="item.profit" required>
    			</div>
  			</div>
			...
			<div class="form-group">
  			...
  			<div class="form-group">
    			<div class="col-sm-offset-2 col-sm-10">
      				<button type="button" class="btn btn-primary" ng-click="addItem()">등록</button>
    			</div>
  			</div>
		</form>
	</div>
